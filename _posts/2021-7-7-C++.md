...
编译器编译前 首先预处理所有以 **#** 开头的预处理语句 header file(.h)不被编译而仅被包含 Symbol in C++ need declaration 
...
组织整个 **Executable(.exe)** 的逻辑有 Symbl and Function Separated in .obj、Main function/Program Entry Point. 对于Entry Point，Project属性 - General - Project defaults - Configuration type - Compiling as application(链接的方式取决于编译文件的类型) 

##### Linking 规则

**c/LINK** ERROR前缀 代表是编译还是链接错误  
**cerror** 语法错误、函数调用未声明(相信已有定义，交给链接器)(声明后不调用可以)(声明和调用必须以某种方式在同一翻译单元中 定义则不用 链接器会找到定义)  
**LINKerror** 无函数进入点、函数调用已声明但找不到定义、函数当前.obj未调用可能在其他.obj中调用已声明但找不到定义、按声明返回类型和参数类型/个数不匹配、同名同参同类型函数分布不同.obj

##### 代码组织方法

**static** 将函数作用范围限定在单.obj文件中 可能在其他文件使用的可能性排除 此时错误变为函数当前.obj已声明未调用找不到定义 改正定义即可  
**inline** 是在函数调用处直接函数体置换 免除了函数调用  
**#include/header file** 中函数定义移动至单一翻译单元 只留声明
